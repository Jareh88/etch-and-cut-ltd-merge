...
def populate_merge_items():
    ...
    INSERT INTO merge_items (
        ...
        product_desc, product_design_listing_no, merge_item_no, order_size,
    ...
    SELECT 
        ...
        (SELECT COALESCE(MAX(merge_item_no), 0) + 1 FROM merge_items WHERE order_no = o.order_no) AS merge_item_no, 
    ...
    FROM order_items o
    LEFT JOIN merge_items m ON o.order_no = m.order_no AND o.order_item_no = m.order_item_no
    WHERE m.order_no IS NULL;
    """
    try:
        ...
        conn.commit()
        conn.close()
        print("✅ Successfully populated merge_items.")
    except Exception as e:
        print(f"❌ Error populating merge_items: {e}")

if __name__ == "__main__":
    populate_merge_items()
