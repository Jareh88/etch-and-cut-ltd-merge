(function () {
	...
	const pattern = /([0-9]{8})[_]+([0-9]{2}[_]+[A-Z]+)[_]+([A-Z]+)[_]+([\w\d]+)[_]+([0-9]{3})/;
	const result  = myPDFFileName.match(pattern);

	var outDate                        = result[1]
	var outProcess                  = result[2] 
	var outMaterialCode         = result[3]
	var outProductTypeCode = result[4]
	var outPlateNum               = result[5]

//	alert(myPDFFileName);
//	alert(result[1] + ',' + result[2] + ',' + result[3] + ',' + result[4]) + ',' + result[5];

	var myPDFExportPath   = "S:/MERGE/OUTPUT/" + outDate + "/" + outMaterialCode + "_" + outProductTypeCode + "/"
	var myPDFFilePathName = myPDFExportPath.concat(outDate, "_", outProcess, "_", outMaterialCode, "_", outProductTypeCode, "_", outPlateNum, ".pdf");
	...
	for(i=0; i<pagein.length; i++){
		...
		if (pageColour == 'GREEN' || pageColour == 'RED'){
			pageout.push(pageNumb);
//			alert(thisPage.label)

			if (pageColour == 'GREEN'){
				myPDFExportPreset = app.pdfExportPresets.item("Adobe PDF Press Quality ZIP");
			} else if (pageColour == 'RED'){
				myPDFExportPreset = app.pdfExportPresets.item("Adobe Trotec Export");
			} else {
				myPDFExportPreset = app.pdfExportPresets.item("Adobe PDF Press Quality ZIP");
			}
		}
